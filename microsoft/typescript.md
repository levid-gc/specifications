...
Microsoft 将在 Open Web Foundation Final Specification Agreement Version 1.0 ("OWF 1.0") as of October 1, 2012 下提供此规范。OWF 1.0 可在[http://www.openwebfoundation.org/legal/the-owf-1-0-agreements/owfa-1-0](http://www.openwebfoundation.org/legal/the-owf-1-0-agreements/owfa-1-0) 查看。

TypeScript 是 Microsoft 公司的商标。
...
&emsp;&emsp;![](../assets/images/typescript/image2.png)
...
&emsp;&emsp;![](../assets/images/typescript/image3.png)
...
&emsp;&emsp;![](../assets/images/typescript/image4.png)
...
类型形参 (type parameter) 表示的是真实的类型，而这个形参与泛型类型引用或泛型函数调用相绑定。类型形参具有约束，可为它们的实际类型实参建立上限。

由于一个类型形参可以表示多个不同的类型实参，因此类型形参相比于其他的类型具有某些特定的限制。特别是，类型形参不能被用作基类或者接口。


### <a name="3.6.1"/>3.6.1 类型形参列表

类，接口，类型别名，以及函数定义都可能可选地在 &lt; 和 > 括号中包含类型形参列表。类型形参同样可以被用在对象，函数，和构造器类型字面量的调用签名中。
